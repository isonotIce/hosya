...
local UPDATE_REPO = "isonotIce/hosya"
local UPDATE_BRANCH = "main"
local UPDATE_RAW_URL = "https://raw.githubusercontent.com/" .. UPDATE_REPO .. "/" .. UPDATE_BRANCH .. "/"
local VERSION_FILE_URL = UPDATE_RAW_URL .. "version.txt"
local SCRIPT_FILE_URL = UPDATE_RAW_URL .. "checkactivity.lua"

local update_checked = false
local update_available = false
local new_version = ""
local update_changelog = ""
local show_update_modal = new.bool(false)
...
local org_9th_point = {
    ...
    ["Армия ЛС"] = "На территории ВЧ минимум 2 сотрудника (не афк без еска)",
...
local max_checks_input = new.int(2)
...
function register_check(org_name)
    ...
    show_chat_notification("info", "Проверка начата", 
        string.format("Организация: %s | Проверок сегодня: %d/%d", org_name, today_checks, config.max_checks_per_day))
end
...
function show_final_report()
    ...
    local online_count = #spisok_org_members
...
function generate_gov_form_text()
    local total_members = #spisok_org_members
    local working_count = 0
    local afk_without_esc_count = 0
    local walked_count = 0
    local working_in_lobby_count = 0
...
function load_config()
    ...
    if file then
        ...
        if content and content ~= "" then
            ...
            config.max_checks_per_day = tonumber(content:match('"max_checks_per_day"%s*:%s*(%d+)')) or 2
...
local function check_admin_level_on_start()
    lua_thread.create(function()
        ...
        if check_my_lvl_adm then
            check_my_lvl_adm = false
            for _, dialogId in ipairs({27091, 8310, 8311, 8312}) do
                pcall(function()
...
local function check_for_updates()
    ...
    lua_thread.create(function()
        ...
        if not http_available then
            show_chat_notification("warning", "Обновление", 
                "Невозможно проверить обновления: отсутствует библиотека socket.http")
            return
        end

        local response, status = http.request(VERSION_FILE_URL)
        
        if not response or status ~= 200 then
            show_chat_notification("warning", "Обновление", 
                "Не удалось проверить обновления: сервер недоступен")
            return
        end

        local online_version = response:match("(%d+%.%d+%.%d+)")
        if not online_version then
            return
        end

        local current_version = script_version():match("(%d+%.%d+%.%d+)") or "1.2.2"
        ...
        local current_parts = version_to_table(current_version)
        local online_parts = version_to_table(online_version)

        update_available = false
        for i = 1, math.max(#current_parts, #online_parts) do
            local cur = current_parts[i] or 0
            local onl = online_parts[i] or 0
        ...
        if update_available then
            show_chat_notification("info", "Доступно обновление", 
                string.format("Новая версия: %s (текущая: %s). Откройте настройки.", 
                    new_version, current_version))
            
            local changelog_url = UPDATE_RAW_URL .. "changelog.txt"
            local changelog_response = http.request(changelog_url)
            if changelog_response and changelog_response:len() > 0 then
                update_changelog = changelog_response
            else
...
local function download_update()
    show_chat_notification("info", "Обновление", "Загрузка новой версии...")
    
    lua_thread.create(function()
        local http_available, http = pcall(require, "socket.http")
        if not http_available then
            show_chat_notification("error", "Обновление", 
                "Ошибка: отсутствует библиотека socket.http")
            return
        end

        local response, status = http.request(SCRIPT_FILE_URL)
        ...
        local current_file = io.open(script_path, "r")
        ...
        if new_file then
            ...
            show_chat_notification("success", "Обновление", 
                string.format("Скрипт обновлен до версии %s! Перезагрузите скрипт.", new_version))

            show_update_modal[0] = false

            lua_thread.create(function()
                wait(3000)
                show_chat_notification("info", "Перезагрузка", 
                    "Для применения обновления перезагрузите скрипт нажав CTRL + R")
            end)
        else
            show_chat_notification("error", "Обновление", 
...
local mainFrame = imgui.OnFrame(
    ...
    function()
        ...
        imgui.SameLine(0, 20)
        imgui.TextColored(ui_theme.success, ru(string.format("Проверок сегодня: %d/%d", 
            today_checks_count, config.max_checks_per_day)))
        
        if last_check_time ~= "" then
            imgui.SameLine(0, 20)
            imgui.TextColored(imgui.ImVec4(0.7, 0.7, 0.7, 1), ru("Последняя: ") .. last_check_time)
        end
        imgui.SameLine(0, 20)
        imgui.TextColored(ui_theme.accent, ru("Твой уровень админки: ") .. my_lvl_adm)
        ...
        if page == 2 then
            ...
            if imgui.Button(button_text, imgui.ImVec2(150, 35)) then
                if not loading_members and selected_org_name ~= ru("Выберите организацию") then
        ...
        elseif page == 4 then
            ...
            imgui.Text(ru("HOSYACheckActivity"))
            imgui.Spacing()
            ...
            imgui.BulletText(ru("Лимит проверок в день: 1-10 раз"))
            ...
            imgui.BulletText(ru("Разрешенное время (MSK): " .. config.check_time_start .. " - " .. config.check_time_end))
            ...
            imgui.BulletText(ru(string.format("Проверок сегодня: %d/%d", today_count, config.max_checks_per_day)))
            if last_time ~= "" then
...
-- ================== ОКНО ОБНОВЛЕНИЯ ==================
local updateModalFrame = imgui.OnFrame(
    function() return show_update_modal[0] end,
    function()
        update_animations()
        ...
        imgui.Begin(ru("Обновление скрипта"), show_update_modal, 
            imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize)
        
        imgui.TextColored(ui_theme.accent, ru("Доступно обновление!"))
        imgui.Separator()
        
        imgui.Spacing()
        imgui.TextColored(ui_theme.success, ru(string.format("Новая версия: %s", new_version)))
        imgui.Text(ru(string.format("Текущая версия: %s", script_version())))
        
        imgui.Spacing()
        imgui.Separator()
        imgui.Spacing()
        
        if update_changelog and update_changelog ~= "" then
            imgui.TextColored(ui_theme.accent, ru("Что нового в обновлении:"))
            imgui.BeginChild(ru("Changelog"), imgui.ImVec2(0, 150), true, imgui.WindowFlags.HorizontalScrollbar)
            imgui.TextWrapped(ru(update_changelog))
            imgui.EndChild()
        else
            imgui.Text(ru("Исправлены ошибки и улучшена стабильность"))
        end
        ...
        imgui.BeginGroup()
        if imgui.Button(ru("Обновить скрипт"), imgui.ImVec2(180, 45)) then
            download_update()
        end
        
        imgui.SameLine()
        
        if imgui.Button(ru("Пропустить эту версию"), imgui.ImVec2(180, 45)) then
            show_update_modal[0] = false
            show_chat_notification("info", "Обновление", 
                "Вы можете обновиться позже через настройки")
        end
        imgui.EndGroup()
        
        imgui.Spacing()
        
        if imgui.Button(ru("Закрыть"), imgui.ImVec2(100, 35)) then
            show_update_modal[0] = false
        end
...
local checkMembersFrame = imgui.OnFrame(
    ...
    function()
        ...
        if #spisok_org_members > 0 and corrent_id_to_rec <= #spisok_org_members then
            ...
            local button_size = imgui.ImVec2(100, 30)
            ...
            if imgui.Button(ru("Работает"), button_size) then
                current.stats = 2
                next_employee()
            end
            ...
            if imgui.Button(ru("АФК без ЕСК"), button_size) then
                current.stats = 3
                next_employee()
            end
            ...
            if imgui.Button(ru("АФК"), button_size) then
                current.stats = 4
                next_employee()
            end
            ...
            if imgui.Button(ru("Личные дела"), button_size) then
                current.stats = 5
                next_employee()
            end
...
local settingsFrame = imgui.OnFrame(
    ...
    function()
        ...
        imgui.TextColored(ui_theme.accent, ru("Лимиты проверок"))
        imgui.Separator()
        
        imgui.Spacing()
        
        imgui.Text(ru("Макс. проверок в день (1-10):"))
        imgui.SetNextItemWidth(120)
        imgui.SameLine()
        if imgui.InputInt(ru("##max_checks"), max_checks_input) then
            config.max_checks_per_day = math.max(1, math.min(10, max_checks_input[0]))
        end
        ...
        imgui.TextColored(ui_theme.accent, ru("Обновления скрипта"))
        imgui.Separator()
        ...
        if imgui.Button(ru("Проверить обновления"), imgui.ImVec2(180, 40)) then
            ...
            check_for_updates()
            show_chat_notification("info", "Проверка обновлений", 
        ...
        if update_available then
            imgui.TextColored(ui_theme.success, ru(string.format("Доступно обновление: %s", new_version)))
            imgui.SameLine()
            if imgui.Button(ru("Установить"), imgui.ImVec2(100, 40)) then
                show_update_modal[0] = true
            end
        else
            imgui.TextColored(imgui.ImVec4(0.7, 0.7, 0.7, 1), ru("У вас последняя версия"))
        end
...
function ev.onShowDialog(dialogId, style, title, button1, button2, text)
    if check_my_lvl_adm then
        if dialogId == 27091 or dialogId == 8310 or dialogId == 8311 or dialogId == 8312 then
            if dialogId == 27091 and button2 == "Закрыть" then
                pcall(function()
                ...
                if result then
                    ...
                    if text:find(mynick, 1, true) then
                        ...
                        lua_thread.create(function()
                            ...
                            wait(100)
                            pcall(function()
                                sampSendDialogResponse(27091, 0, 1, "")
                            end)
                        end)
                        return false
                    end
    ...
    local success, err = pcall(function()
        ...
        sampRegisterChatCommand("hosya_stats", function()
            ...
            show_chat_notification("info", "Статистика проверок", 
                string.format("Проверок сегодня: %d/%d | Последняя: %s", 
                    today_count, config.max_checks_per_day, 
                    last_time ~= "" and last_time or "не было"))
        end)

        sampRegisterChatCommand("hosya_update", function()
            update_checked = false
            check_for_updates()
            show_chat_notification("info", "Проверка обновлений", 
                "Проверяем наличие обновлений...")
        ...
        show_chat_notification("info", "Загрузка", "HOSYACheckActivity загружен.")
        ...
        while true do
            ...
            if renderMembersWindow[0] then
                ...
                if wasKeyPressed(vkeys.VK_1) and not imgui.GetIO().WantCaptureKeyboard then
                    if corrent_id_to_rec <= #spisok_org_members then
                        spisok_org_members[corrent_id_to_rec].stats = 2
                        next_employee()
                    end
                elseif wasKeyPressed(vkeys.VK_2) and not imgui.GetIO().WantCaptureKeyboard then
                    if corrent_id_to_rec <= #spisok_org_members then
